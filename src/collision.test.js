import checkCollision, { checkHasCollisionObject, collided,removeCollision } from './collision'
test('does bar have collision object', () => {


    let otherbar = { id: "other", column: 1, row: 1, length: 2 }




    expect(otherbar).not.toMatchObject({ collisions: {} })
    expect(checkHasCollisionObject(otherbar)).toEqual(expect.objectContaining({ collisions: {} }));
...
test('collision added keys to collision object', () => {
    let firstbar = { id: "first" }
    let otherbar = { id: "other" }
...
test('no collision remove keys from collision object', () => {
    let firstbar = { id: "first",collision:{ other: '' } }
    let otherbar = { id: "other",collision:{ first: '' } }
