...
export default function Reserver(props) {
    ...
    return <div role="grid" onMouseLeave={props.mouseLeaveGrid} style={{ ...props.style, position: "relative" }} >
        ...
        {[...Array(rowCount)].map((x, r) => {
            return <div role="rowgroup" className={styles.row} key={r} style={{ height: props.dimension, display: "flex" }}>
                <div className={styles.row_cell} style={{ width: props.rowTitleWidth, height: props.dimension + "px" }} >
                    {rowTitles[r]}
                </div>
                {([...Array(columnCount)].map((x, c) => {
                    return <div role="gridcell" aria-colindex={c} onDragOver={(e) => { e.preventDefault(); props.mouseDragOverCell({ cell: { row: r, column: c } }) }}
