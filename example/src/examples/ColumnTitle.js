        ...
        <>
            ...
            <Reserver
                ...
                columnTitleRow={(columnCount) => {
                    return dateRange(new Date(), columnCount, 'days').map((val, index) => {         
                        console.log(index)            
                        return <div style={{ background: "#12D3CF", height: "100%", textAlign: "center" }} onClick={() => { setSelectedDate(resolveDate(new Date(), index, 'days', "dddd, MMMM Do YYYY, h:mm:ss a")) }}>{val}</div>
                    })
                }}
